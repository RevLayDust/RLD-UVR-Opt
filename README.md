...
### Runtime Model Memory

- MDX-Net ONNX FP16 conversion is performed in CPU RAM and passed directly to ONNX Runtime as serialized bytes. UVR does not create or update persistent `.fp16.onnx` conversion files.
- The active ONNX Runtime/PyTorch session is cached while the same model and precision remain selected.
- Starting processing with another model or precision evicts the previous cached session before the replacement is loaded, then releases Python references and unused CUDA allocator memory.
- Legacy `.fp16.onnx` files are ignored and left untouched; UVR neither depends on nor automatically deletes them.
